 ...
 */
export const getDepthTier = async (depth: number) => {
...
function calculateLevelUp(currentLevel: number, currentExp: number, gainedExp: number) {
  ...
  while (exp >= level * 100) {
    ...
    level += 1;
    levelGain += 1;
    if (levelGain > 100) break; // Safety break
  }
 ...
 */
function calculateDamage(attackerStats: any, defenderDef: number, isGathering = false) {
...
export async function generateGatheringEncounter(character: Character) {
  ...
  if (availableNodes.length === 0) availableNodes = nodes;
...
function getPlayerClass(stats: any): "WARRIOR" | "ARCHER" | "MAGE" {
  if (!stats) return "WARRIOR";
  ...
  if (code.startsWith("WARRIOR") || code === "EXCALIBUR" || code === "CHAOS_BLADE") return "WARRIOR";
  if (code.startsWith("ARCHER") || code === "ARTEMIS_BOW") return "ARCHER";
  if (code.startsWith("MAGE") || code === "MERLIN_STAFF") return "MAGE";
...
export async function executeCombat(character: Character, enemy: any) {
  ...
  while (playerHp > 0 && enemyHp > 0) {
    ...
    if (!turnActionTaken) {
      ...
      if (stats.lifesteal > 0 && playerDamage > 0) {
        ...
        if (heal > 0) playerHp = Math.min(character.maxHp, playerHp + heal);
      }
      let msg = "";
      if (isCrit) {
        const critMsgs = [
  ...
  if (isWin && monsterTemplate) {
    ...
    if (stats.goldBonus > 0) finalGold = Math.floor(finalGold * (1 + stats.goldBonus / 100));
  }

  // 📈 EXP bonus from gear
  if (stats.equipExpBonus > 0) finalExp = Math.floor(finalExp * (1 + stats.equipExpBonus / 100));
