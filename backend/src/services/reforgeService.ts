...
export class ReforgeService {
   ...
   */
  async reforgeItem(characterId: string, inventoryItemId: string) {
    ...
    if (item.characterId !== characterId) throw new Error("Item does not belong to you");
    if (item.template.type !== "EQUIPMENT") throw new Error("Can only reforge equipment");

    // 2. Calculate Costs (Scales with Rarity & Level)
    const rarityRank = (item.template as any).rarity?.rank || 1;
  ...
  private generateRolls(playerStats: any, template: any) {
