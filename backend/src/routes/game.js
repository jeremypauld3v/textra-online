...
export async function gameRoutes(server) {
    ...
    server.post("/resolve-encounter", async (request, reply) => {
        ...
        try {
            ...
            if (action === "skip") {
                ...
                if (encounter.type === "PVP" || encounter.type === "PVP_INCOMING") {
                    ...
                    if (Math.random() < fleeChance) {
                        ...
                        if (character.previousStatus?.startsWith("TRAVELING"))
                            await addTravelJob(characterId);
                        ...
                        if (encounter.type === "PVP_INCOMING" && encounter.targetId) {
                            ...
                            if (p2) {
                                const resumeStatus = p2.previousStatus || "IDLE";
                                await prisma.character.update({
                                    where: { id: encounter.targetId },
                                    data: { actionStatus: resumeStatus, pendingEncounter: Prisma.DbNull, previousStatus: null }
                                });
                                if (resumeStatus.startsWith("TRAVELING"))
                                    await addTravelJob(encounter.targetId);
                            }
                            const io = getIO();
            ...
            if (action === "attack" && encounter.type === "PVP") {
                ...
                await prisma.character.update({
                    where: { id: characterId },
                    data: { pendingEncounter: waitingEncounter }
                });
            ...
            if (action === "attack" && (encounter.type === "PVP_INCOMING" || encounter.type === "PVP_WAITING")) {
                ...
                const p2Enc = p2State?.pendingEncounter;
                const p1Enc = p1State?.pendingEncounter;
                ...
                const winnerPrevStatus = combatResult.isWin ? p2State?.previousStatus : p1State?.previousStatus;
    ...
    server.post("/stats/allocate", async (request, reply) => {
        const { characterId } = request.user;
        const { stat, amount = 1 } = request.body;
    ...
    server.post("/friends/accept", async (request, reply) => {
        const { characterId } = request.user;
        const { targetUserId } = request.body; // targetUserId here is actually characterId
        try {
    ...
    server.get("/chat/private/recent", async (request, reply) => {
        ...
        try {
            ...
            const partners = Array.from(partnersMap.values()).sort((a, b) => new Date(b.lastMessageAt).getTime() - new Date(a.lastMessageAt).getTime());
